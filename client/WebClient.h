...
namespace WebAccounts {
    ...
    */
    ...
    private:
        ...
        Cookie loginCookie; //!< Session data is stored in a cookie after logging in
        std::string cookieSessionName; //!< Name of the cookie attribute with session data
        WebClientPimpl* privImpl;

        const std::string GetSessionCookieStr() const;
        void SetServerCookie(Cookie&& cookie);
    public:
        /*! \brief Only constructor for the web client. It must have a version, domain URL, and port #
            ...
            \param version. String e.g. "1"
            \param cookie. String of the cookie with session data. e.g. "WebServerSession"

            Will create a new HTTP client object for `client`
            isOK is set to TRUE 
        */
        EXPORT_DLL WebClient(const char* domain, int port, const char* version, const char* cookie);
